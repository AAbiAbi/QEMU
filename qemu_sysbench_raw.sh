...
# 主机上保存结果的目录
RESULTS_DIR="/Users/a25076/Desktop/241CloudComputing/QEMU/result_raw_1"
mkdir -p "${RESULTS_DIR}"
...
do
    ...
    do
        ...
        do
            ...
            result=$(ssh -o StrictHostKeyChecking=no -i "$SSH_PRIVATE_KEY" -p 10023 "$SSH_USER@localhost" "$test_cmd")
...
clear_cache() {
  ...
  echo '19980903' |ssh -o StrictHostKeyChecking=no -i "$SSH_KEY_PATH" -p 2223 "$SSH_USER@localhost" "sudo -S sh -c 'echo 3 > /proc/sys/vm/drop_caches'"
}
